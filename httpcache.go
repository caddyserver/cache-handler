...
func (s *SouinCaddyMiddleware) FromApp(app *SouinApp) error {
	...
	if app.GetMappingEvictionInterval() == 0 {
		if s.Configuration.DefaultCache.GetMappingEvictionInterval() == 0 {
			app.MappingEvictionInterval = configurationtypes.Duration{Duration: time.Hour}
		}
	}
...
func (s *SouinCaddyMiddleware) Provision(ctx caddy.Context) error {
	...
	if app.SurrogateStorage == nil {
		app.withSurrogateStorer(s.SurrogateKeyStorer)
	} else {
